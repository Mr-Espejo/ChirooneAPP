...
export const syncMediaTaskLogic = async (itemId?: string) => {
...
export const startVideoGenerationTaskLogic = async (itemId?: string) => {
  const { prisma } = await import("../utils/prisma.js");
  const { kieaiService } = await import("../integrations/kieaiService.js");
...
export const startVideoGenerationCron = schedules.task({
  ...
  cron: "15,45 * * * *",
  run: startVideoGenerationTaskLogic
});
...
export const highFrequencyPublishingCron = schedules.task({
  ...
  cron: "*/15 * * * *",
  run: async () => {
    const { publishingService } = await import("../modules/publishing/publishing.service.js");
